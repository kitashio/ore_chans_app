...
final postStreamProvider = StreamProvider.autoDispose<List<Post>>((ref) {
  ...
  final snapshot = ref
      ...
      .collection('post')
      .snapshots(); // snapshotで全てのデータを取得できる。
  return snapshot.map((snapshot) {
    // ここで、QuerySnapshotを取得する。
    return snapshot.docs.map((doc) {
      // ここで、QuerySnapshotからQueryDocumentSnapshotを取得する。
...
final poseRefStreamProvider = Provider.autoDispose((ref) {
  return ref.watch(fireStoreProvider).collection('post').withConverter<Post?>(
      // Post? とすることでデータがない場合はnullを返す
      fromFirestore: (ds, _) {
